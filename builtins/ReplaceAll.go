...
func ReplaceAll(e *engine.Evaluator, c *engine.Context, args []core.Expr) core.Expr {
	...
	if rulesList, ok := rule.(core.List); ok && rulesList.Length() > 0 && rulesList.Head() == symbol.List {
...
func replaceAllRecursive(expr core.Expr, rule core.Expr) core.Expr {
	...
	} else if rulesList, ok := rule.(core.List); ok && rulesList.Length() > 0 && rulesList.Head() == symbol.List {
